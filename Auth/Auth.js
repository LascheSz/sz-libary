...
exports.register = async (req, res) => {
    ...
    try {
        ...
        res.cookie("jwt", token, {
            ...
            maxAge: maxAge * 1000 // Gültigkeitsdauer in Millisekunden
        });

        console.log("Benutzerrolle nach der Registrierung:", user.role);

        // Erfolgsantwort zurückgeben
        return res.status(201).json({ message: "Benutzer erfolgreich registriert", role: user.role });
        console.log("Benutzerrolle nach der Registrierung:", user.role);
    } catch (e) {
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (!foundUser) {
            return res.status(401).json({ message: "Login Fehlerhaft! Benutzer nicht gefunden" });
        }
        const isMatch = await bcrypt.compare(password, foundUser.password);
        if (!isMatch) {
            return res.status(401).json({ message: "Login Fehlerhaft! Passwort inkorrekt" });
        }
        ...
        // Erfolgsantwort zurückgeben
        res.status(200).json({ message: "Login erfolgreich", role: foundUser.role });
    } catch (e) {
